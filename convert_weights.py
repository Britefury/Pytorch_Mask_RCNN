...
@click.command()
@click.argument('dest_path', type=click.Path(file_okay=True, dir_okay=False, writable=True))
@click.argument('src_path', type=click.Path(file_okay=True, dir_okay=False, exists=True))
def convert(dest_path, src_path):
    import datetime
    import glob
    import itertools
    import json
    ...
    state_dict = get_state_dict(src_path)

    new_state_dict = make_name_right(state_dict, model)
    torch.save(new_state_dict, dest_path)
